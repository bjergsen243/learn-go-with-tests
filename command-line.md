# Command line và cấu trúc dự án

**[Tất cả code của chương này được lưu tại đây](https://github.com/quii/learn-go-with-tests/tree/main/command-line)**

Product owner của chúng ta bây giờ muốn *thay đổi hướng đi* bằng cách giới thiệu một ứng dụng thứ hai - một ứng dụng dòng lệnh.
...
Product owner muốn cơ sở dữ liệu được chia sẻ giữa hai ứng dụng để bảng xếp hạng cập nhật theo các trận thắng được ghi lại trong ứng dụng mới.

## Nhắc lại về code

Chúng ta có một ứng dụng với tệp `main.go` khởi chạy một máy chủ HTTP. Máy chủ HTTP sẽ không thú vị đối với chúng ta trong bài tập này nhưng abstraction mà nó sử dụng thì có. Nó phụ thuộc vào một `PlayerStore`.
...
## Refactor dự án trước tiên

Dự án của chúng ta bây giờ cần tạo ra hai binary, máy chủ web hiện có và ứng dụng CLI.

Trước khi bắt tay vào công việc mới, chúng ta nên cấu trúc lại dự án của mình để phù hợp với việc này.

Cho đến nay, tất cả code đều nằm trong một thư mục, trong một đường dẫn trông như thế này:

`$GOPATH/src/github.com/your-name/my-app`

Để bạn có thể tạo một ứng dụng trong Go, bạn cần một hàm `main` bên trong một `package main`. Cho đến nay, tất cả domain code của chúng ta đều nằm bên trong `package main` và `func main` của chúng ta có thể tham chiếu đến mọi thứ.

Điều này vẫn ổn cho đến nay và là một phương pháp hay khi không quá sa đà vào package structure. Nếu bạn dành thời gian xem qua thư viện tiêu chuẩn, bạn sẽ thấy rất ít các thư mục và cấu trúc phức tạp.
...
Hiện tại chúng ta đã thực hiện việc tách biệt hiệu quả giữa ứng dụng và code thư viện nhưng bây giờ chúng ta cần thay đổi một số tên gói. Hãy nhớ rằng khi bạn xây dựng một ứng dụng Go, gói của nó *phải* là `main`.

Thay đổi tất cả các code khác để có một gói tên là `poker`.

Cuối cùng, chúng ta cần import gói này vào `main.go` để có thể sử dụng nó nhằm tạo máy chủ web của mình. Sau đó, chúng ta có thể sử dụng code thư viện của mình bằng cách sử dụng `poker.TenHam`.
...
Đường dẫn đầy đủ có vẻ hơi rắc rối, nhưng đây là cách bạn có thể nhập *bất kỳ* thư viện công khai nào vào code của mình.

Bằng cách tách biệt domain code của chúng ta thành một gói riêng biệt và đẩy nó lên một kho lưu trữ công khai như GitHub, bất kỳ nhà phát triển Go nào cũng có thể viết code của riêng họ để nhập gói đó và sử dụng các tính năng mà chúng ta đã viết. Lần đầu tiên bạn thử chạy nó, nó sẽ phàn nàn rằng tệp không tồn tại nhưng tất cả những gì bạn cần làm là chạy `go get`.
...
Trước khi bắt tay vào viết các test, hãy thêm một ứng dụng mới mà dự án của chúng ta sẽ xây dựng. Tạo một thư mục khác bên trong `cmd` gọi là `cli` và thêm một `main.go` với nội dung sau:
...
Chúng ta biết mình cần tạo ra một thứ gọi là `CLI` cho phép chúng ta `Play` poker. Nó sẽ cần đọc đầu vào của người dùng và sau đó ghi lại các trận thắng vào một `PlayerStore`.

Tuy nhiên, trước khi đi quá xa, hãy viết một test để kiểm tra xem nó có tích hợp với `PlayerStore` như chúng ta mong muốn hay không.
...
- Chúng ta có thể sử dụng `StubPlayerStore` từ các test khác
...
Bạn sẽ kết thúc với code như thế này:
...
Hãy nhớ rằng chúng ta chỉ đang cố gắng làm cho test chạy để có thể kiểm tra xem test có thất bại như chúng ta mong đợi hay không:
...
Hãy mở rộng test của chúng ta để thực hiện việc này.
...
Chúng ta tạo một `io.Reader` trong test của mình bằng cách sử dụng `strings.NewReader` tiện dụng, làm đầy nó bằng những gì chúng ta mong đợi người dùng sẽ nhập.
...
Test vượt qua. Tiếp theo chúng ta sẽ thêm một test khác để buộc mình phải viết một số code thực sự, nhưng trước tiên, hãy refactor.
...
Test bây giờ nên trông như sau:
...
Bây giờ hãy viết một test *khác* với đầu vào người dùng khác để buộc chúng ta thực sự phải đọc nó.
...
Bây giờ khi chúng ta đã có một số test vượt qua, chúng ta nên kết nối mọi thứ vào `main`. Hãy nhớ rằng chúng ta nên luôn cố gắng có phần mềm hoạt động được tích hợp đầy đủ càng nhanh càng tốt.

Trong `main.go`, hãy thêm đoạn code sau và chạy nó. (bạn có thể phải điều chỉnh đường dẫn của sự phụ thuộc thứ hai để khớp với đường dẫn trên máy tính của bạn)
...
Những gì đang xảy ra ở đây là do chúng ta đang cố gắng gán cho các trường `playerStore` và `in` trong `CLI`. Đây là các trường unexported (riêng tư). Chúng ta *có thể* làm điều này trong code test của mình vì test của chúng ta nằm trong cùng một gói với `CLI` (`poker`). Nhưng `main` của chúng ta nằm trong gói `main` nên nó không có quyền truy cập.
...
Trong tất cả các ví dụ khác cho đến nay, khi chúng ta tạo một tệp test, chúng ta khai báo nó nằm trong cùng một gói với gói mà chúng ta đang test.

Điều này vẫn ổn và nó có nghĩa là trong những dịp hiếm hoi khi chúng ta muốn test thứ gì đó bên trong gói, chúng ta có quyền truy cập vào các kiểu unexported.

Nhưng vì chúng ta đã ủng hộ việc *không* test các thứ bên trong *nói chung*, liệu Go có thể giúp thực thi điều đó không? Điều gì sẽ xảy ra nếu chúng ta có thể test code của mình ở nơi mà chúng ta chỉ có quyền truy cập vào các kiểu exported (giống như `main` của chúng ta đã làm)?

Khi bạn viết một dự án với nhiều gói, tôi khuyên bạn nên đặt tên gói test có hậu tố `_test`. Khi bạn làm điều này, bạn sẽ chỉ có quyền truy cập vào các kiểu công khai trong gói của mình. Điều này sẽ giúp ích trong trường hợp cụ thể này nhưng cũng giúp thực thi kỷ luật chỉ test các API công khai. Nếu bạn vẫn muốn test các nội dung bên trong, bạn có thể tạo một test riêng biệt với gói bạn muốn test.

Có một câu nói với TDD là nếu bạn không thể test code của mình thì có lẽ người dùng code của bạn cũng khó tích hợp với nó. Sử dụng `package foo_test` sẽ giúp giải quyết vấn đề này bằng cách buộc bạn phải test code của mình như thể bạn đang nhập nó giống như người dùng gói của bạn sẽ làm.

Trước khi sửa lỗi `main`, hãy đổi gói của test bên trong `CLI_test.go` thành `poker_test`.
...
Bây giờ chúng ta đã vấp phải nhiều câu hỏi hơn về thiết kế gói. Để test phần mềm của mình, chúng ta đã tạo ra các stub unexported và các hàm helper mà hiện tại không còn khả dụng cho chúng ta sử dụng trong `CLI_test` vì các helper được định nghĩa trong các tệp `_test.go` trong gói `poker`.

#### Chúng ta có muốn các stub và helper của mình là 'công khai' không?

Đây là một cuộc thảo luận mang tính chủ quan. Một số người có thể lập luận rằng bạn không muốn làm ô nhiễm API của gói mình bằng các code để phục vụ các test.

Trong bài trình bày ["Advanced Testing with Go"](https://speakerdeck.com/mitchellh/advanced-testing-with-go?slide=53) của Mitchell Hashimoto, có mô tả cách tại HashiCorp, họ ủng hộ việc làm điều này để người dùng gói có thể viết các test mà không cần phải phát minh lại các stub. Trong trường hợp của chúng ta, điều này có nghĩa là bất kỳ ai sử dụng gói `poker` của chúng ta sẽ không phải tự tạo ra stub `PlayerStore` của riêng họ nếu họ muốn làm việc với code của chúng ta.
...
Bạn sẽ cần làm cho các helper trở thành công khai (hãy nhớ rằng việc export được thực hiện bằng một chữ cái viết hoa ở đầu) nếu bạn muốn chúng được lộ ra cho những người nhập gói của chúng ta.

Trong test `CLI`, bạn sẽ cần gọi code như thể bạn đang sử dụng nó trong một gói khác.
...
Bằng cách làm này, chúng ta có thể đơn giản hóa và refactor code đọc:
...
Thay đổi test để sử dụng constructor thay thế và chúng ta sẽ quay lại tình trạng các test vượt qua.
...
Bây giờ hãy refactor cả hai ứng dụng của chúng ta để sử dụng hàm này nhằm tạo kho lưu trữ.
...
Chương này có nghĩa là chúng ta muốn tạo hai ứng dụng, tái sử dụng domain code mà chúng ta đã viết cho đến nay. Để làm được điều này, chúng ta cần cập nhật cấu trúc gói của mình để có các thư mục riêng biệt cho các hàm `main` tương ứng.

Bằng cách này, chúng ta đã gặp phải các vấn đề tích hợp do các giá trị unexported, vì vậy điều này chứng minh thêm giá trị của việc làm việc theo từng "lát" nhỏ và tích hợp thường xuyên.

Chúng ta đã học cách `mypackage_test` giúp chúng ta tạo một môi trường test giống như trải nghiệm đối với các gói khác khi tích hợp với code của bạn, nhằm giúp bạn nắm bắt các vấn đề tích hợp và xem code của bạn dễ hoạt động như thế nào (hoặc không!).
...
### Abstraction đơn giản giúp tái sử dụng code dễ hơn

Hầu như không mất công sức để tích hợp `PlayerStore` vào ứng dụng mới của chúng ta (sau khi chúng ta đã thực hiện các điều chỉnh về gói) và việc test sau đó cũng rất dễ dàng vì chúng ta đã quyết định để lộ phiên bản stub của mình.
