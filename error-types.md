...
Như Pedro đã nói, chúng ta _có thể_ viết một bài test cho status error như sau.
...
> _Tôi_ muốn sử dụng code này như thế nào?
...
Thay vì kiểm tra chuỗi ký tự chính xác của lỗi, chúng ta đang thực hiện một [type assertion](https://tour.golang.org/methods/15) trên lỗi để xem nó có phải là một `BadStatusError` hay không. Điều này thể hiện mong muốn của chúng ta về _loại_ lỗi một cách rõ ràng hơn. Giả sử assertion thành công, chúng ta có thể kiểm tra các thuộc tính của lỗi xem chúng có chính xác không.
...
- Các bài test của chúng ta giờ đây phản ánh (và làm tài liệu) những gì người dùng code _có thể_ làm nếu họ quyết định muốn xử lý lỗi tinh vi hơn là chỉ logging. Chỉ cần thực hiện một type assertion và sau đó bạn có thể dễ dàng truy cập vào các thuộc tính của lỗi.
...
Hãy luôn đảm bảo các bài test phản ánh cách _bạn_ muốn sử dụng code, vì vậy hãy cân nhắc việc tạo các error type để đóng gói các loại lỗi. Điều này giúp việc xử lý các loại lỗi khác nhau trở nên dễ dàng hơn cho người dùng code, đồng thời giúp code xử lý lỗi đơn giản và dễ đọc hơn.
