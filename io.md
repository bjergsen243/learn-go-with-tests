...
[Trong chương trước](json.md), chúng ta đã tiếp tục lặp lại ứng dụng của mình bằng cách thêm một endpoint mới `/league`. Trong quá trình đó, chúng ta đã tìm hiểu về cách xử lý JSON, embedding và routing.

Product owner lo lắng vì điểm số bị mất khi khởi động lại server — do store đang lưu in-memory. Cô ấy cũng muốn endpoint `/league` trả về người chơi được sắp xếp theo số trận thắng!
...
Nó sẽ không mở rộng đặc biệt tốt nhưng vì đây là một prototype nên nó sẽ ổn cho lúc này. Nếu hoàn cảnh của chúng ta thay đổi và nó không còn phù hợp nữa, việc đổi nó sang một thứ khác sẽ đơn giản nhờ vào abstraction `PlayerStore` mà chúng ta đã sử dụng.

Chúng ta sẽ giữ lại `InMemoryPlayerStore` trong lúc này để các test tích hợp tiếp tục vượt qua khi chúng ta phát triển kho lưu trữ mới của mình. Một khi chúng ta tự tin rằng triển khai mới của mình đủ để làm cho test tích hợp vượt qua, chúng ta sẽ thay thế nó và sau đó xóa `InMemoryPlayerStore`.

## Viết test trước tiên

Đến bây giờ bạn chắc hẳn đã quen thuộc với các interface xung quanh thư viện tiêu chuẩn để đọc dữ liệu (`io.Reader`), ghi dữ liệu (`io.Writer`) và cách chúng ta có thể sử dụng thư viện tiêu chuẩn để test các hàm này mà không cần sử dụng các tệp thật.

Để công việc này hoàn tất, chúng ta sẽ cần triển khai `PlayerStore`, vì vậy chúng ta sẽ viết các test cho kho lưu trữ của mình bằng cách gọi các phương thức cần triển khai. Chúng ta sẽ bắt đầu với `GetLeague`.
...
Chúng ta *đã* làm việc này trước đây! Code test cho máy chủ của chúng ta đã phải giải mã JSON từ phản hồi.

Hãy thử làm cho code gọn gàng hơn (DRY) bằng một hàm.

Tạo một tệp mới tên là `league.go` và đưa code này vào:
...
Gọi hàm này trong implementation của chúng ta và trong helper test `getLeagueFromResponse` trong `server_test.go`.
...
Có một lỗ hổng trong implementation của chúng ta. Đầu tiên, hãy nhắc nhở bản thân cách `io.Reader` được định nghĩa:
...
Thêm đoạn code sau vào cuối test hiện tại của chúng ta:
...
Chúng ta muốn phần này vượt qua, nhưng nếu bạn chạy test thì nó không vượt qua.
...
Bạn còn nhớ về embedding chứ? Đây là một interface bao gồm `Reader` và [`Seeker`](https://golang.org/pkg/io/#Seeker).
...
Thử chạy test, nó hiện đã vượt qua! Thật may cho chúng ta là `strings.NewReader` mà chúng ta đã sử dụng trong test của mình cũng triển khai `ReadSeeker` nên chúng ta không phải thực hiện bất kỳ thay đổi nào khác.
...
Chúng ta cần thêm phương thức vào kiểu mới của mình để test có thể biên dịch.
...
Bây giờ nó đã biên dịch và test thất bại:
...
Bạn đã thấy hàng tá các bản refactor helper test rồi, vì vậy tôi sẽ để việc làm cho nó hoạt động lại cho bạn:
...
- Tạo một tệp tạm thời cho mỗi test. `*os.File` triển khai `ReadWriteSeeker`. Ưu điểm của việc này là nó trở thành một test tích hợp nhiều hơn, chúng ta thực sự đang đọc và ghi từ hệ thống tệp nên nó sẽ mang lại cho chúng ta mức độ tin cậy rất cao. Nhược điểm là chúng ta thích các test đơn vị hơn vì chúng nhanh hơn và thường đơn giản hơn. Chúng ta cũng sẽ cần phải làm việc nhiều hơn xung quanh việc tạo các tệp tạm thời và sau đó đảm bảo chúng được xóa sau test.
...
Tôi không nghĩ có một câu trả lời nào là đặc biệt sai ở đây, nhưng bằng cách chọn sử dụng một thư viện bên thứ ba, tôi sẽ phải giải thích về dependency management! Vì vậy, chúng ta sẽ sử dụng các tệp thay thế.

Trước khi thêm test của mình, chúng ta cần làm cho các test khác biên dịch bằng cách thay thế `strings.Reader` bằng một `os.File`.

Hãy tạo một số hàm helper sẽ tạo một tệp tạm thời với một số dữ liệu bên trong và abstract các test điểm số của chúng ta:
...
Bạn sẽ nhận thấy chúng ta không chỉ trả về `ReadWriteSeeker` (tệp) mà còn trả về một hàm. Chúng ta cần đảm bảo rằng tệp được xóa sau khi test kết thúc. Chúng ta không muốn để lộ chi tiết của các tệp vào test vì nó dễ dẫn đến lỗi và không thú vị cho người đọc. Bằng cách trả về một hàm `removeFile`, chúng ta có thể lo liệu các chi tiết trong helper của mình và tất cả những gì người gọi phải làm là chạy `defer cleanDatabase()`.
...
Chạy các test và chúng sẽ vượt qua! Có một lượng kha khá các thay đổi nhưng hiện tại có cảm giác như chúng ta đã hoàn thành định nghĩa interface của mình và việc thêm các test mới từ giờ sẽ rất dễ dàng.
...
Implementation của chúng ta trống rỗng nên số điểm cũ vẫn được trả về.
...
Khi bạn `range` qua một slice, bạn sẽ nhận được chỉ số hiện tại của vòng lặp (trong trường hợp của chúng ta là `i`) và một *bản sao* của phần tử tại chỉ số đó. Việc thay đổi giá trị `Wins` của một bản sao sẽ không có bất kỳ tác động nào đến slice `league` mà chúng ta lặp qua. Vì lý do đó, chúng ta cần lấy tham chiếu đến giá trị thực tế bằng cách thực hiện `league[i]` và sau đó thay đổi giá trị đó.

Nếu bạn chạy các test, chúng sẽ vượt qua.

## Refactor

Trong `GetPlayerScore` và `RecordWin`, chúng ta đang lặp qua biểu thức `[]Player` để tìm một người chơi theo tên.

Chúng ta có thể refactor code chung này trong các chi tiết nội bộ của `FileSystemStore`, nhưng đối với tôi, có vẻ như đây là code có khả năng hữu ích mà chúng ta có thể đưa vào một kiểu mới. Làm việc với một "League" cho đến nay luôn là với `[]Player` nhưng chúng ta có thể tạo một kiểu mới gọi là `League`. Điều này sẽ dễ dàng hơn cho các nhà phát triển khác hiểu và sau đó chúng ta có thể gắn các phương thức hữu ích vào kiểu đó để sử dụng.

Bên trong `league.go`, hãy thêm đoạn code sau:
...
Thay đổi interface `PlayerStore` của chúng ta để trả về `League` thay vì `[]Player`. Thử chạy lại các test, bạn sẽ gặp vấn đề biên dịch vì chúng ta đã thay đổi interface nhưng nó rất dễ khắc phục; chỉ cần thay đổi kiểu trả về từ `[]Player` thành `League`.
...
Điều này trông tốt hơn nhiều và chúng ta có thể thấy cách chúng ta có thể tìm thấy các chức năng hữu ích khác xung quanh `League` để refactor.
...
Happy path trông khá ổn vì vậy chúng ta có thể thử sử dụng `Store` mới của mình trong test tích hợp. Điều này sẽ mang lại cho chúng ta sự tin cậy hơn rằng phần mềm hoạt động và sau đó chúng ta có thể xóa `InMemoryPlayerStore` dư thừa.
...
Nếu bạn chạy test, nó sẽ vượt qua và giờ chúng ta có thể xóa `InMemoryPlayerStore`. `main.go` bây giờ sẽ có vấn đề biên dịch, điều này sẽ thúc đẩy chúng ta sử dụng kho lưu trữ mới trong code "thực".
...
## Refactor thêm và các mối quan tâm về performance
...
Test sẽ thất bại với lỗi phân tích cú pháp JSON.
...
Lưu ý: Việc ép kiểu `file.(*os.File)` có thể gây ra lỗi nếu chúng ta truyền một thứ gì đó không phải là tệp. Trong code thực tế, bạn nên kiểm tra điều đó.
...
Test thất bại vì thứ tự hiện tại là Cleo rồi đến Chris.
...
`sort.Slice` nhận một slice và một hàm so sánh. Hàm so sánh này trả về `true` nếu phần tử tại `i` nên đứng trước phần tử tại `j`. Ở đây chúng ta muốn sắp xếp giảm dần theo số trận thắng.
...
- **Refactor code thông minh**. Tách biệt logic xử lý dữ liệu (`League`) khỏi logic lưu trữ (`FileSystemPlayerStore`).
- **Sắp xếp trong Go**. Sử dụng `sort.Slice` để sắp xếp các slice dữ liệu dựa trên các quy tắc tùy chỉnh.
