...
Giả sử bạn đang làm việc với một người bạn để tạo một phần mềm blog. Ý tưởng là tác giả sẽ viết các bài đăng của họ bằng markdown, với một số metadata ở đầu file. Khi khởi động, web server sẽ đọc một thư mục để tạo ra các `Post`, và sau đó một hàm `NewHandler` riêng biệt sẽ sử dụng các `Post` đó làm nguồn dữ liệu cho web server của blog.
...
Thay vào đó, hãy cố mang lại giá trị *thực* cho người dùng càng nhanh càng tốt theo happy path. Khi đã có giá trị end-to-end, việc bổ sung các yêu cầu còn lại thường đơn giản hơn nhiều.
...
Để viết một test xung quanh việc này, chúng ta cần một loại thư mục test với một số bài đăng mẫu trong đó. *Cũng không có gì quá sai trái với điều này*, nhưng bạn đang thực hiện một số đánh đổi:

- đối với mỗi test, bạn có thể cần tạo các file mới để test một hành vi cụ thể
- một số hành vi sẽ khó test, chẳng hạn như lỗi khi tải file
- các test sẽ chạy chậm hơn một chút vì chúng cần truy cập vào hệ thống file
...
Điều này cho phép chúng ta nới lỏng sự ràng buộc của mình vào một hệ thống file cụ thể, từ đó cho phép chúng ta inject các triển khai khác nhau tùy theo nhu cầu của mình.
...
Nếu chúng ta sử dụng interface này, người dùng package của chúng ta có một số tùy chọn được tích hợp sẵn trong thư viện tiêu chuẩn để sử dụng. Học cách tận dụng các interface được định nghĩa trong thư viện tiêu chuẩn của Go (ví dụ: `io.fs`, [`io.Reader`](https://golang.org/pkg/io/#Reader), [`io.Writer`](https://golang.org/pkg/io/#Writer)), là điều quan trọng để viết các package loosely coupled. Các package này sau đó có thể được tái sử dụng trong các ngữ cảnh khác với những gì bạn tưởng tượng ban đầu, với sự phiền hà tối thiểu từ người tiêu dùng của bạn.

Trong trường hợp của chúng ta, có lẽ người tiêu dùng muốn các bài đăng được nhúng vào file nhị phân Go thay vì các file trong một hệ thống file "thật"? Dù thế nào đi nữa, *code của chúng ta không cần phải quan tâm*.

Đối với các test của chúng ta, package [testing/fstest](https://golang.org/pkg/testing/fstest/) cung cấp cho chúng ta một triển khai của [io/FS](https://golang.org/pkg/io/fs/#FS) để sử dụng, tương tự như các công cụ mà chúng ta đã quen thuộc trong [net/http/httptest](https://golang.org/pkg/net/http/httptest/).
...
Lưu ý rằng package của test là `blogposts_test`. Hãy nhớ rằng, khi TDD được thực hành tốt, chúng ta thực hiện theo phương pháp *consumer-driven*: chúng ta không muốn test các chi tiết nội bộ vì *người tiêu dùng* không quan tâm đến chúng. Bằng cách thêm `_test` vào tên package dự định của mình, chúng ta chỉ truy cập vào các thành viên được export từ package của mình - giống như một người dùng thực sự của package.
...
> MapFS là một hệ thống file đơn giản trong bộ nhớ để sử dụng trong các test, được đại diện dưới dạng một map từ tên đường dẫn (các đối số cho Open) đến thông tin về các file hoặc thư mục mà chúng đại diện.

Điều này có vẻ đơn giản hơn việc duy trì một thư mục chứa các file test, và nó sẽ thực thi nhanh hơn.
...
Package không tồn tại. Tạo một file mới `blogposts.go` và đặt `package blogposts` vào bên trong. Sau đó, bạn sẽ cần import package đó vào các test của mình. Đối với tôi, các bản import bây giờ trông như thế này:
...
Bây giờ các test sẽ không biên dịch được vì package mới của chúng ta không có hàm `NewPostsFromFS` trả về một loại tập hợp nào đó.

```
./blogpost_test.go:16:12: undefined: blogposts.NewPostsFromFS
```

Điều này buộc chúng ta phải tạo bộ khung cho hàm của mình để test có thể chạy. Hãy nhớ đừng quá suy nghĩ về code tại thời điểm này; chúng ta chỉ đang cố gắng có một test đang chạy, và để đảm bảo nó thất bại như chúng ta mong đợi. Nếu chúng ta bỏ qua bước này, chúng ta có thể bỏ qua các giả định và viết một test không hữu ích.
...
Test bây giờ sẽ thất bại một cách chính xác:
...
> Sliming rất hữu ích để tạo "bộ khung" cho đối tượng của bạn. Thiết kế một interface và thực thi logic là hai mối quan tâm khác nhau, và việc test slime một cách chiến thuật cho phép bạn tập trung vào từng thứ một.
...
Ngay lập tức, cái nhìn lý tưởng hóa của chúng ta về thế giới đã bị cản trở vì các lỗi có thể xảy ra, nhưng hãy nhớ bây giờ trọng tâm của chúng ta là *làm cho test vượt qua*, không phải thay đổi thiết kế, vì vậy chúng ta sẽ tạm bỏ qua lỗi lúc này.

Phần còn lại của code khá đơn giản: lặp lại qua các entry, tạo một `Post` cho mỗi mục và trả về slice.

## Refactor

Mặc dù các test của chúng ta đang vượt qua, chúng ta không thể sử dụng package mới này bên ngoài ngữ cảnh này, bởi vì nó bị ràng buộc vào một triển khai cụ thể `fstest.MapFS`. Nhưng, nó không nhất thiết phải như vậy. Hãy thay đổi đối số cho hàm `NewPostsFromFS` của chúng ta để chấp nhận interface từ thư viện tiêu chuẩn.
...
Chạy test: nó sẽ phàn nàn về số lượng giá trị trả về sai. Việc sửa code khá đơn giản.
...
Điều này sẽ làm cho test vượt qua. Người thực hành TDD trong bạn có thể thấy khó chịu vì chúng ta không thấy một test thất bại trước khi viết code để lan truyền lỗi từ `fs.ReadDir`. Để làm điều này "đúng đắn", chúng ta sẽ cần một test mới nơi chúng ta truyền vào một `fs.FS` giả (test double) bị lỗi để làm cho `fs.ReadDir` trả về một `error`.
...
Điều này sẽ mang lại cho bạn sự tự tin vào cách tiếp cận của chúng ta. Interface mà chúng ta đang sử dụng chỉ có một phương thức, điều này làm cho việc tạo các test-double để test các kịch bản khác nhau trở nên dễ dàng.

Trong một số trường hợp, test việc xử lý lỗi là điều thực tế nên làm, nhưng trong trường hợp của chúng ta, chúng ta không làm bất cứ điều gì *thú vị* với lỗi đó, chúng ta chỉ lan truyền nó, vì vậy việc viết một test mới là không đáng công sức.

Về logic, các lần lặp lại tiếp theo của chúng ta sẽ xoay quanh việc mở rộng kiểu `Post` để nó có một số dữ liệu hữu ích.

## Viết test trước tiên

Chúng ta sẽ bắt đầu với dòng đầu tiên trong sơ đồ bài đăng blog dự kiến, trường title.

Chúng ta cần thay đổi nội dung của các file test sao cho chúng khớp với những gì đã chỉ định, sau đó chúng ta có thể thực hiện một assertion rằng nó được phân tích chính xác.
...
func TestNewBlogPosts(t *testing.T) {
	...
	// phần còn lại của code test được lược bỏ cho ngắn gọn
	got := posts[0]
...
Thêm trường mới vào kiểu `Post` của chúng ta để test có thể chạy
...
Chạy lại test, và bạn sẽ nhận được một kết quả thất bại rõ ràng
...
Lưu ý rằng trọng tâm của chúng ta tại thời điểm này không phải là viết code đẹp đẽ, mà chỉ là đạt được một điểm mà chúng ta có phần mềm hoạt động.

Mặc dù điều này mang lại cảm giác là một bước tiến nhỏ nhưng nó vẫn yêu cầu chúng ta viết một lượng code đáng kể và đưa ra một số giả định về cách xử lý lỗi. Đây sẽ là thời điểm bạn nên nói chuyện với đồng nghiệp của mình và quyết định cách tiếp cận tốt nhất.
...
`fs.FS` cung cấp cho chúng ta một cách để mở một file bên trong nó theo tên bằng phương thức `Open`. Từ đó, chúng ta đọc dữ liệu từ file và hiện tại, chúng ta không cần bất kỳ phân tích phức tạp nào, chỉ cần cắt bỏ phần văn bản `Title: ` bằng cách slicing chuỗi.

## Refactor

Việc tách đoạn 'mã mở file' khỏi đoạn 'mã phân tích nội dung file' sẽ làm cho code trở nên đơn giản hơn để hiểu và làm việc.
...
Khi bạn tách ra các hàm hoặc phương thức mới, hãy cẩn thận và suy nghĩ về các đối số. Bạn đang thiết kế ở đây, và bạn có một sự tự do để suy nghĩ sâu sắc về những gì là phù hợp vì bạn có các test đã vượt qua. Hãy nghĩ về coupling và cohesion. Trong trường hợp này, bạn nên tự hỏi bản thân:
...
Bạn có thể đưa ra một lập luận tương tự cho hàm `getPost`, hàm này nhận vào một đối số `fs.DirEntry` nhưng chỉ gọi `Name()` để lấy tên file. Chúng ta không cần tất cả những thứ đó; hãy tách rời khỏi kiểu dữ liệu đó và truyền tên file qua dưới dạng một chuỗi. Đây là code đã được refactor hoàn chỉnh:
...
Chúng ta cũng nên chăm sóc các test của mình. Chúng ta sẽ thực hiện nhiều xác nhận trên `Post`, vì vậy chúng ta nên viết một số code để trợ giúp việc đó
...
Hãy mở rộng test hơn nữa để trích xuất dòng tiếp theo từ file, phần description. Cho đến khi làm cho nó vượt qua, mọi thứ lúc này sẽ mang lại cảm giác thoải mái và quen thuộc.
...
Test lúc này có thể biên dịch được, và thất bại.
...
Chúng ta có sự lặp lại xung quanh việc quét một dòng và sau đó đọc văn bản. Chúng ta biết mình sẽ thực hiện thao tác này ít nhất một lần nữa, đây là một lần refactor đơn giản để DRY code, hãy bắt đầu với điều đó.
...
Điều này hầu như không tiết kiệm được dòng mã nào, nhưng đó hiếm khi là mục tiêu của việc refactor. Những gì tôi đang cố gắng làm ở đây chỉ là tách biệt phần *cái gì* khỏi phần *làm thế nào* của việc đọc các dòng để làm cho code mang tính declarative hơn một chút đối với người đọc.
...
Bây giờ khi tôi đang nhìn vào code với tư duy refactor sáng tạo, tôi muốn thử làm cho hàm `readLine` của mình đảm nhận việc loại bỏ thẻ. Cũng có một cách dễ đọc hơn để cắt bỏ tiền tố khỏi một chuỗi bằng hàm `strings.TrimPrefix`.
...
Bạn có thể thích ý tưởng này hoặc không, nhưng tôi thì thích. Vấn đề là ở giai đoạn refactor, chúng ta được tự do chơi đùa với các chi tiết bên trong, và bạn có thể tiếp tục chạy các test để kiểm tra xem mọi thứ vẫn hoạt động chính xác hay không. Chúng ta luôn có thể quay lại các trạng thái trước đó nếu không hài lòng. Cách tiếp cận TDD mang lại cho chúng ta quyền được thường xuyên thử nghiệm các ý tưởng, nhờ đó có nhiều cơ hội hơn để viết được code tuyệt vời.

Yêu cầu tiếp theo là trích xuất các tag của bài đăng. Nếu bạn đang theo dõi, tôi khuyên bạn nên tự mình triển khai nó trước khi đọc tiếp. Bây giờ bạn đã có một nhịp độ lặp lại tốt và cảm thấy tự tin để trích xuất dòng tiếp theo và phân tích dữ liệu.

Để ngắn gọn, tôi sẽ không đi qua các bước TDD, nhưng đây là test với các thẻ được thêm vào.
...
Bạn sẽ chỉ đang tự lừa dối mình nếu chỉ sao chép và dán những gì tôi viết. Để đảm bảo tất cả chúng ta đều chung một quan điểm, đây là code bao gồm việc trích xuất các thẻ.
...
Lần lặp lại cuối cùng trên happy-path của chúng ta là trích xuất phần body.
...
Thay đổi dữ liệu test để có dấu phân cách, và một phần nội dung với một vài dòng mới để kiểm tra xem chúng ta có lấy được tất cả nội dung không.
...
Thêm `Body` vào `Post` và test sẽ thất bại.
...
Tuy nhiên, quan trọng nhất là chúng ta có phần mềm hoạt động, và chúng ta đã định nghĩa interface của mình. Những điều trên chỉ là các lần lặp lại tiếp theo, thêm nhiều test hơn để viết và thúc đẩy hành vi của chúng ta. Để hỗ trợ bất kỳ hành vi nào ở trên, chúng ta không cần phải thay đổi *thiết kế*, chỉ cần các chi tiết thực thi.
...
`fs.FS`, và những thay đổi khác trong Go 1.16 mang lại cho chúng ta một số cách trang nhã để đọc dữ liệu từ hệ thống file và test chúng một cách đơn giản.

Nếu bạn muốn thử nghiệm code "thật":
...
Lưu ý sự đối xứng giữa code sản xuất
...
Đây là khi phương pháp TDD hướng người tiêu dùng, top-down *cảm thấy đúng đắn*.

Người dùng package của chúng ta có thể nhìn vào các test của chúng ta và nhanh chóng nắm bắt được nó phải làm gì và cách sử dụng nó như thế nào. Với tư cách là những người bảo trì, chúng ta có thể *tự tin rằng các test của mình là hữu ích vì chúng được đứng từ quan điểm của người tiêu dùng*. Chúng ta không test các chi tiết thực thi hoặc các chi tiết phụ khác, vì vậy chúng ta có thể tin tưởng một cách hợp lý rằng các test của mình sẽ giúp ích, thay vì cản trở chúng ta khi refactor.

Bằng cách dựa trên các thực hành kỹ thuật phần mềm tốt như [**Dependency Injection**](dependency-injection.md), code của chúng ta trở nên đơn giản để test và tái sử dụng.

Khi bạn tạo các package, ngay cả khi chúng chỉ là nội bộ trong dự án của bạn, hãy ưu tiên cách tiếp cận hướng người tiêu dùng từ trên xuống dưới. Điều này sẽ ngăn bạn tưởng tượng quá mức các thiết kế và tạo ra các sự trừu tượng có thể bạn thậm chí không cần, và sẽ giúp đảm bảo các test bạn viết là hữu ích.
