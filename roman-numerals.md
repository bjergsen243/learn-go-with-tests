...
Chưa có gì nhiều để refactor.

*Tôi biết* cảm giác thật kỳ lạ khi chỉ hard-code kết quả, nhưng với TDD chúng ta muốn thoát khỏi trạng thái "đỏ" càng nhanh càng tốt. Có vẻ như chúng ta chưa hoàn thành được gì nhiều nhưng chúng ta đã định nghĩa được API và có một test nắm bắt được một trong các quy tắc của mình; ngay cả khi mã "thực sự" trông khá ngớ ngẩn.
...
Chúng ta có thể sử dụng subtests để nhóm các test một cách đẹp mắt.
...
Chúng ta thấy có sự lặp lại trong các test. Khi bạn đang kiểm thử một thứ gì đó có vẻ như là vấn đề "với đầu vào X, chúng ta mong đợi Y", có lẽ bạn nên sử dụng table-driven test.
...
Bây giờ chúng ta có thể dễ dàng thêm nhiều trường hợp hơn mà không cần phải viết thêm bất kỳ test code rườm rà nào.
...
Được rồi, tôi bắt đầu thấy không thích những câu lệnh if này và nếu bạn quan sát kỹ code, bạn có thể thấy rằng chúng ta đang xây dựng một chuỗi các ký tự `I` dựa trên giá trị của `arabic`.

Chúng ta "biết" rằng đối với các con số phức tạp hơn, chúng ta sẽ thực hiện một số loại phép tính số học và nối chuỗi.

Hãy thử refactor với những suy nghĩ này trong đầu, nó *có thể không* phù hợp cho giải pháp cuối cùng nhưng không sao cả. Chúng ta luôn có thể vứt bỏ mã của mình và bắt đầu lại từ đầu với các test mà chúng ta có để dẫn dắt.
...
Thông thường, tôi sẽ không bận tâm đến những tối ưu hóa như vậy cho đến khi gặp vấn đề thực tế về hiệu suất, nhưng lượng code không lớn hơn nhiều so với việc nối chuỗi "thủ công", vì vậy chúng ta cũng có thể sử dụng cách tiếp cận nhanh hơn này.

Code trông có vẻ tốt hơn và mô tả được domain *như chúng ta biết ngay bây giờ*.
...
- Dựa trên các tín hiệu tôi đọc được từ code, được thúc đẩy từ các test của chúng ta qua một số kịch bản rất cơ bản, tôi có thể thấy rằng để xây dựng một Chữ số La Mã, tôi cần trừ đi từ `arabic` khi áp dụng các biểu tượng.
...
Tôi khá chắc chắn cách tiếp cận này cũng sẽ hợp lệ cho các số 6 (VI), 7 (VII) và 8 (VIII). Tuy nhiên, hãy thêm các trường hợp vào bộ test của chúng ta và kiểm tra (tôi sẽ không đưa code vào đây cho ngắn gọn, hãy kiểm tra github để xem các mẫu nếu bạn không chắc chắn).
...
Cảm giác như code vẫn đang mách bảo rằng có một sự refactor nào đó ở đâu đó nhưng tôi chưa thấy rõ, vì vậy hãy tiếp tục.

Tôi cũng sẽ bỏ qua code cho phần này, nhưng hãy thêm vào các kịch test của bạn một test cho số `10` vốn nên là `X` và làm cho nó vượt qua trước khi đọc tiếp.

Dưới đây là một vài test tôi đã thêm vào vì tôi tin tưởng rằng code của chúng ta nên hoạt động cho đến số 39:
...
Nếu bạn đã từng lập trình hướng đối tượng (OOP), bạn sẽ biết rằng bạn nên nhìn các câu lệnh `switch` với một chút nghi ngờ. Thường thì bạn đang nắm bắt một khái niệm hoặc dữ liệu trong một đoạn imperative code, trong khi thực tế nó có thể được nắm bắt trong một class structure thay thế.
...
Chúng ta có thể refactor điều này bằng cách tách biệt dữ liệu khỏi hành vi.
...
Dưới đây là một số test case, hãy thử làm cho chúng vượt qua.
...
Hãy áp dụng cùng một cách tiếp cận cho các biểu tượng còn lại, chỉ cần thêm dữ liệu vào cả các test và mảng các biểu tượng của chúng ta.

Code của bạn có hoạt động cho số `1984`: `MCMLXXXIV` không?
...
Chúng ta có thể sử dụng lại các kịch test của mình ở đây với một chút refactor.
...
Test bây giờ sẽ chạy được và thất bại:
...
Tiếp theo, hãy thay đổi chỉ số slice trong test của chúng ta để chuyển sang kịch bản tiếp theo (ví dụ: `cases[:2]`). Hãy tự mình làm cho nó vượt qua bằng code ngớ ngẩn nhất mà bạn có thể nghĩ ra, tiếp tục viết mã ngớ ngẩn (cuốn sách hay nhất từ trước đến nay phải không nào?) cho cả trường hợp thứ ba. Đây là mã ngớ ngẩn của tôi:
...
Thông qua sự ngớ ngẩn của *code thực sự hoạt động*, chúng ta bắt đầu thấy một mẫu hình giống như trước đây. Chúng ta cần lặp qua đầu vào và xây dựng một thứ gì đó, trong trường hợp này là một tổng giá trị.
...
Hàm `HasPrefix(s, prefix)` kiểm tra xem chuỗi `s` có bắt đầu bằng `prefix` hay không và `TrimPrefix(s, prefix)` gỡ bỏ `prefix` khỏi `s`, để chúng ta có thể tiến hành với các biểu tượng chữ số La Mã còn lại. Nó hoạt động với `IV` và tất cả các test case khác.
...
## Giới thiệu về property-based testing

Đã có một vài quy tắc trong domain về Chữ số La Mã mà chúng ta đã làm việc trong chương này:
...
Các test chúng ta đã viết cho đến nay có thể được mô tả là example based tests, nơi chúng ta cung cấp *ví dụ* cho test framework để xác minh.

Điều gì sẽ xảy ra nếu chúng ta có thể lấy những quy tắc mà mình biết về lĩnh vực này và bằng cách nào đó thử nghiệm chúng dựa trên code của mình?

Kiểm thử dựa trên thuộc tính giúp bạn thực hiện điều này bằng cách đưa dữ liệu ngẫu nhiên vào code và xác minh xem các quy tắc bạn mô tả có luôn đúng hay không. Nhiều người nghĩ rằng property based testing chủ yếu về dữ liệu ngẫu nhiên nhưng họ đã lầm. Thử thách thực sự đối với property based testing là phải có một hiểu biết *tốt* về domain để bạn có thể viết các thuộc tính này.

Nói đủ rồi, hãy xem một số code:
...
Test đầu tiên của chúng ta sẽ kiểm tra xem nếu chúng ta chuyển đổi một số thành chữ số La Mã, khi chúng ta sử dụng hàm khác để chuyển đổi nó trở lại thành một con số thì chúng ta sẽ nhận được kết quả ban đầu của mình.
...
Chuyện gì đang xảy ra? Hãy thử thêm đoạn sau vào code xác nhận:
...
Điều này thật tuyệt vời! Chúng ta đã bị buộc phải suy nghĩ sâu sắc hơn về domain của mình, đây là một sức mạnh thực sự của property based testing.
...
Go có các kiểu dữ liệu cho *unsigned integers* (số nguyên không dấu), nghĩa là chúng không thể âm; điều đó giúp loại bỏ một loại bug trong code của chúng ta ngay lập tức. Bằng cách thêm số 16, nó có nghĩa đó là một số nguyên 16 bit có thể lưu trữ tối đa `65535`, vẫn còn quá lớn nhưng giúp chúng ta tiến gần hơn đến những gì mình cần.

Thử cập nhật code để sử dụng `uint16` thay vì `int`. Tôi đã cập nhật `assertion` trong test để hiển thị rõ ràng hơn một chút.
...
Nếu bạn chạy test, bây giờ chúng thực sự chạy và bạn có thể thấy những gì đang được kiểm thử. Bạn có thể chạy nhiều lần để thấy code của chúng ta đứng vững trước các giá trị khác nhau! Điều này cho tôi rất nhiều sự tự tin rằng mã của mình đang hoạt động như mong muốn.
...
- Bạn có thể viết các property based test để kiểm tra các thuộc tính khác mà chúng ta đã mô tả không?
...
Suy nghĩ về việc viết code chuyển đổi số 1984 thành MCMLXXXIV có khiến bạn cảm thấy e sợ ngay từ đầu không? Với tôi thì có, và tôi đã viết phần mềm khá lâu rồi.

Bí quyết, như mọi khi, là **bắt đầu với một thứ gì đó đơn giản** và thực hiện các **bước nhỏ**.

Tại bất kỳ thời điểm nào trong quy trình này, chúng ta không có những bước nhảy vọt lớn, không thực hiện bất kỳ đợt refactor khổng lồ nào, hay rơi vào một mớ hỗn độn.
...
- Nếu bạn có thể nghĩ ra cách mô tả các quy tắc domain trong code, chúng là một công cụ tuyệt vời mang lại cho bạn nhiều sự tự tin hơn.
- Buộc bạn phải suy nghĩ về domain của mình một cách sâu sắc.
