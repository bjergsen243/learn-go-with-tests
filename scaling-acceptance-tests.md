...
Chương này là phần tiếp theo của chương [Giới thiệu về Acceptance Tests](https://quii.gitbook.io/learn-go-with-tests/testing-fundamentals/intro-to-acceptance-tests). Bạn có thể tìm thấy [code hoàn chỉnh cho chương này trên GitHub](https://github.com/quii/go-specs-greet).
...
Cuối cùng, tôi và [Riya Dattani](https://twitter.com/dattaniriya) đã thảo luận về chủ đề này trong bối cảnh BDD qua buổi nói chuyện [Acceptance tests, BDD and Go](https://www.youtube.com/watch?v=ZMWJCk_0WrY).
...
Qua nhiều năm, tôi đã làm việc với rất nhiều công ty và đội ngũ phát triển. Tất cả đều nhận thấy nhu cầu cần có acceptance tests - một hình thức nào đó để kiểm tra xem hệ thống có hoạt động đúng hay không từ góc nhìn người dùng. Tuy nhiên, vấn đề truyền thống là các acceptance tests thường trở thành gánh nặng cho đội phát triển:
...
- **Thay đổi hành vi bên ngoài**. Khi tính năng nghiệp vụ thay đổi, việc cập nhật acceptance tests là hợp lý và cần thiết.
- **Thay đổi implementation detail hoặc refactor**. Những thay đổi này không nên buộc bạn phải sửa test.
...
Chúng ta muốn acceptance tests chỉ thay đổi khi hành vi của hệ thống thay đổi, chứ không phải khi implementation details thay đổi. Cách hợp lý nhất là tách chúng thành hai phần riêng biệt.

### Các loại complexity

Với vai trò kỹ sư, chúng ta phải đối mặt với hai loại phức tạp:

- **Accidental complexity** - phức tạp phát sinh từ việc làm việc với máy tính: mạng, ổ đĩa, API, v.v.
- **Essential complexity** - còn gọi là "domain logic". Đây là các quy tắc và nguyên lý mà lĩnh vực kinh doanh yêu cầu.
...
Theo lời Dave Farley trong video ở trên, và qua thảo luận của Riya và tôi, chúng ta nên viết các bài test dưới dạng **specifications**. Specification mô tả hành vi mà chúng ta mong muốn, không đề cập đến accidental complexity hay implementation details.

Điều này nghe quen thuộc. Trong quá trình phát triển sản phẩm, chúng ta luôn cố gắng tách biệt trách nhiệm cho các module. Chúng ta sử dụng `interface` để tách HTTP handler khỏi những thứ không liên quan đến HTTP. Vậy hãy áp dụng tư duy tương tự cho acceptance tests.
...
Khi chúng ta tách biệt specification khỏi implementation details, chúng ta có thể tái sử dụng specification trong nhiều bối cảnh khác nhau.

#### Cấu hình linh hoạt cho driver
...
#### Sử dụng các driver khác nhau để test các phần khác nhau
...
- Lý tưởng hơn nữa, tôi muốn viết domain code để có thể chạy specification dưới dạng unit tests. Điều này mang lại phản hồi cực nhanh, xác nhận rằng essential complexity được triển khai đúng.

### Acceptance tests thay đổi vì lý do đúng đắn
...
Khi ứng dụng phát triển, bạn sẽ nhận ra mình có thể tái sử dụng các driver cho nhiều test. Mỗi khi implementation details thay đổi, bạn chỉ cần sửa ở một điểm duy nhất.

Nếu làm đúng cách, bạn đạt được sự tách biệt hoàn toàn giữa implementation details và specification. Cách tổ chức này tạo ra một cấu trúc đơn giản và rõ ràng để quản lý sự thay đổi - điều cốt lõi khi hệ thống phát triển lớn hơn.

### Acceptance tests như phương pháp phát triển phần mềm
...
Theo hướng tiếp cận top-down, chúng ta bắt đầu với acceptance test - bài test kiểm tra hành vi từ bên ngoài. Bài test này đóng vai trò "north star" để định hướng công việc. Hãy tập trung để bài test này pass. Ban đầu, nó sẽ fail một thời gian trong khi bạn viết đủ code cần thiết.
...
#### Hiểm họa của cách tiếp cận bottom-up

Rõ ràng là cách tiếp cận top-down khác với bottom-up. Cách bottom-up có ưu điểm riêng, nhưng cũng tiềm ẩn rủi ro. Khi bạn xây dựng các service và viết code mà chưa tích hợp vào ứng dụng tổng thể và chưa được kiểm tra bằng acceptance test, **bạn có nguy cơ lãng phí công sức vào những thứ dựa trên giả định chưa được kiểm chứng**.
...
- Làm những thứ không cần thiết
- Khó tích hợp
- Phải viết lại rất nhiều

Đó chính là lãng phí.

## Đủ lý thuyết, hãy bắt đầu viết code
...
Tại thời điểm này, có thể có người cho rằng việc tách specification khỏi implementation là "trừu tượng quá mức". **Tôi cam đoan rằng, acceptance tests bị ràng buộc chặt với implementation sẽ trở thành gánh nặng cho cả đội phát triển**. Theo kinh nghiệm của tôi, phần lớn acceptance tests tốn kém bảo trì là do chúng bị ràng buộc chặt với implementation details, chứ không phải vì trừu tượng quá mức.
...
Bước đầu tiên - tạo một bài black-box test có khả năng tự động build, chạy và dọn dẹp - thường tốn nhiều công sức. Nhưng việc làm điều này ngay từ đầu dự án sẽ đơn giản hơn rất nhiều. Cá nhân tôi, mỗi dự án mới, tôi đều bắt đầu bằng việc thiết lập server "hello world" cùng với acceptance test.
...
> Commit bao nhiêu "tội" cũng được miễn là test pass
...
Bước khởi đầu tốn nhiều công sức. Chúng ta tạo ra nhiều file Go để xây dựng và test một HTTP handler chỉ trả về chuỗi cố định. Dù iteration 0 có phần nặng nề, nhưng nó sẽ phục vụ tốt cho các iteration tiếp theo.

Việc thay đổi chức năng giờ đây không quá khó - chỉ cần cập nhật specification rồi xử lý các thay đổi kéo theo. Giờ chúng ta đã có Dockerfile và Testcontainers cho acceptance test, bạn không cần lo lắng về infrastructure nữa.
...
Nhưng liệu chúng ta có thể tái sử dụng specification ở đây không? Vì specification được thiết kế để ẩn đi implementation details, nếu nó nắm bắt được essential complexity và domain code cũng thể hiện đúng essential complexity đó, chúng ta hoàn toàn có thể kết nối chúng.
...
Sau iteration đầu tiên tốn nhiều công sức, các iteration tiếp theo dễ dàng hơn vì specification, driver và implementation đã được tách biệt. Thay đổi specification chỉ yêu cầu cập nhật driver và implementation, không ảnh hưởng đến infrastructure (container, Docker).

Dù mất thêm chi phí build Docker image và khởi tạo container, chúng ta có feedback loop cho phép test **toàn bộ** ứng dụng:
...
### Làm cho thay đổi trở nên dễ dàng

Đôi khi, bạn cần refactor _trước khi_ thêm chức năng mới.

> Đầu tiên hãy làm cho thay đổi trở nên dễ dàng, rồi mới thực hiện thay đổi đó
...
[Test Pyramid](https://martinfowler.com/articles/practical-test-pyramid.html) gợi ý cách kết hợp hợp lý các loại test. Bạn nên đọc bài của Fowler để hiểu chi tiết. Tóm lại: nên có nhiều unit tests và ít acceptance tests.
...
Hãy thực hiện từng bước nhỏ, commit thường xuyên sau mỗi lần test pass. Nếu gặp khó khăn, tham khảo [code trên GitHub](https://github.com/quii/go-specs-greet).
...
Xây dựng hệ thống với chi phí thay đổi hợp lý đòi hỏi thiết kế acceptance tests để hỗ trợ bạn, không phải trở thành gánh nặng bảo trì. Bạn có thể dùng chúng như kim chỉ nam, hoặc như GOOS nói, để "grow" phần mềm một cách có phương pháp.

Hy vọng qua ví dụ này, bạn thấy cách chúng ta thay đổi ứng dụng bằng một workflow có cấu trúc và dễ dự đoán, và cách ứng dụng phát triển một cách có tổ chức.

Hãy suy nghĩ về việc thu thập yêu cầu khi muốn mở rộng dự án. Tập trung vào domain, tạo specification không phụ thuộc vào implementation để dùng làm kim chỉ nam cho cả đội. Riya và tôi có thảo luận về cách dùng BDD và "Example Mapping" [trong buổi nói chuyện tại GopherconUK](https://www.youtube.com/watch?v=ZMWJCk_0WrY) để nắm bắt essential complexity và viết specification hiệu quả.

Tách biệt essential complexity và accidental complexity sẽ mang lại cấu trúc rõ ràng và có chủ đích cho quy trình làm việc. Điều này giúp acceptance tests trở nên linh hoạt và giảm maintenance burden.
