# Tại sao cần unit test và cách tận dụng chúng hiệu quả
...
- Công cụ hỗ trợ testing, benchmarking, linting và shipping của Go thuộc hàng tốt nhất so với hầu hết các lựa chọn khác.
- Thư viện chuẩn cực kỳ mạnh mẽ.
- Tốc độ biên dịch nhanh, giúp rút ngắn feedback loop.
- Cam kết về khả năng tương thích ngược của Go: Ngay cả khi Go sắp có generics và nhiều tính năng khác, các nhà thiết kế hứa rằng code Go viết 5 năm trước vẫn có thể biên dịch được. Tác giả từng mất hàng tuần để nâng cấp một dự án từ Scala 2.8 lên 2.10.
...
- Cải thiện code mà không thay đổi hành vi của chương trình.
- Chia nhỏ thành từng bước nhỏ, mỗi bước đều có thể kiểm tra ngay.
- Dựa vào test tự động để đảm bảo không làm hỏng hệ thống.
...
- Refactoring giúp code dễ bảo trì và mở rộng hơn.
...
- Quá nhiều câu lệnh `if`, làm cho code trở nên khó đọc và bảo trì.
...
Những ai không viết test thường phải test thủ công, điều này tốn thời gian và không thể mở rộng.
...
- Cung cấp phản hồi nhanh hơn nhiều so với test thủ công
...
Vài tuần sau, “Định luật Thay đổi Liên tục” xuất hiện và một lập trình viên mới vào dự án. Cô ấy cho rằng hình vuông nên được tạo từ hai hình chữ nhật thay vì hai tam giác.
...
- Vì vậy, chúng không lý tưởng cho việc refactor vì feedback loop quá chậm.
...
**CÓ**. Unit test là test các “unit” trong hệ thống, như cách tôi đã mô tả trước đó. Unit test **KHÔNG** đơn giản là test một class, function hay một đoạn code cụ thể. Chúng kiểm tra một **đơn vị** có **ý nghĩa trong domain**, với API rõ ràng và không ràng buộc vào cách triển khai bên trong.
...
Chúng ta có thể thấy rằng Refactoring, Unit Test, và Thiết kế Unit tốt bổ trợ lẫn nhau để giúp phần mềm dễ bảo trì và mở rộng.

### Refactoring

- Cho ta thấy chất lượng của unit test. Nếu sau khi refactor, ta phải test thủ công, nghĩa là unit test chưa đủ. Nếu test bị fail mà không rõ lý do, có thể test đang ở mức trừu tượng sai (hoặc thậm chí không có giá trị và nên bị xóa).
- Giúp quản lý độ phức tạp bên trong và giữa các module.

### Unit tests

- Là “tấm lưới an toàn” giúp ta refactor mà không lo làm hỏng chức năng
...
- Dễ dàng refactor mà không cần sửa đổi quá nhiều test.

Làm thế nào để duy trì khả năng refactor liên tục?
...
Agile dạy chúng ta rằng phần mềm nên được phát triển theo hướng iterative, bắt đầu nhỏ, nhận phản hồi nhanh từ người dùng, rồi tiến hóa dần dần. **TDD giúp thực thi triết lý này trong code**.
...
- Viết unit test tốt là một bài toán thiết kế. Hãy tổ chức code theo các unit có ý nghĩa, có thể kết hợp như các viên gạch Lego.
