...
### Hệ thống của bạn là tổ hợp các modules

Liên hệ lại với ý tưởng của Farley, tôi muốn nói về **incrementalism**. Xây dựng phần mềm là quá trình *học hỏi liên tục*. Không thực tế khi yêu cầu nhóm phải hiểu trọn vẹn toàn bộ requirements để deliver value ngay từ đầu. Thay vào đó, chúng ta phải liên tục tối ưu hệ thống để **gather feedback quickly and experiment**.

Bạn cần một **modular system** để phát huy lợi thế từ những khái niệm trong chương này. Code module gắn với fakes đáng tin cậy cho phép bạn experiment trên hệ thống qua automated tests với chi phí cực kỳ thấp.

Chúng tôi nhận thấy việc chuyển đổi kịch bản giả định (nhưng có khả năng xảy ra) thành self-contained tests trở nên rất dễ dàng. Điều này giúp hiểu vấn đề sâu hơn và phát triển phần mềm vững chắc hơn bằng cách lắp ghép modules, chạy thử với dữ liệu đa dạng, hoặc mô phỏng API bị lỗi, v.v.

Các module được thiết kế rõ ràng và test tốt cho phép bạn nâng cấp hệ thống liên tục mà không cần hiểu _tất cả mọi thứ_ cùng một lúc.

### Nhưng tôi đang làm việc trên một dự án nhỏ với các API ổn định

Ngay cả với API ổn định, bạn cũng không nên để DX, builds, v.v. bị tightly coupled với code của người khác. Khi áp dụng đúng cách, bạn sẽ có bộ modules lắp ráp tùy ý để build cho production, chạy local, hoặc viết các loại test khác nhau với test doubles đáng tin cậy.

Cách tiếp cận này cho phép bạn cô lập phần hệ thống bạn quan tâm và viết test có ý nghĩa tập trung vào vấn đề thực sự mà nhóm đang cần giải quyết.

### Dependencies là first-class citizens

Stubs hay spies vẫn có chỗ đứng. Mô phỏng ad-hoc behaviour của dependencies trong test vẫn hữu dụng, nhưng hãy cẩn thận đừng để chi phí bảo trì vượt tầm kiểm soát.
...
Nhiều đội quá phụ thuộc vào shared environment để test. Nhược điểm là feedback không **isolated** và **chậm**. Bạn cũng không thể dễ dàng tạo ra các experiments đa dạng để kiểm tra cách hệ thống hoạt động với các dependencies khác nhau.

**Chúng ta cần mô hình hóa dependencies tốt hơn** để có thể test và experiment trên máy local trước khi đưa lên production. Hãy tạo fakes chân thực cho dependencies, bảo đảm chất lượng bằng contracts. Khi đã có nền tảng này, bạn có thể viết ngày càng nhiều test với các kịch bản đa dạng trên hệ thống của mình.
